...
//メインのためテストはしないでください
//ピン番号は本番仕様
//アクセルとブレーキの二つ音波センサと初期設定適用
//コントローラ化済み
//
//初期設定テスト ->      ctrl_set_test
//二つ距離センサテスト->  sonicmeasure2
//加速度センサのテスト->  
//前バージョンのソース->  wheelctrl2
//メモ
//タスクリスト
//mixed:ペダル踏み込み時、計測可能最短距離以下で異常動作
//      対応策：（い）ハード的改善、最短距離で計測しないように。
//             （ろ）ソフト的対応、異常値を検出したら＞不適。
//ソフト:gセンサ値をスティック値へ(要実験)
//ハード:gセンサ実装
//program by ryk
